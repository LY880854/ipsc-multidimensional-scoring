...
The label TSV contains one row per selected sample, in the intended output order. It requires `sample_id` and either `observed_label` or the historical column name `routeA_label`; `group` and `fold` are retained when present. The supplied `results/fold_assignment.tsv` can be used as this table.
